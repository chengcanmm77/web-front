...
(function() {
    ...
    J = {
        ...
        list_s: function(j) {
            ...
            while (E--) {
                U = J.list(B[E]),
                i.push('<a href="' + U.url + '" class="' + U.cls + '">' + U.name + "</a>")
            }
    ...
    Q = function(j, n, m, B) {
        ...
        var W = (function() {
            ...
            q = k.length,
            t = {},
            r = []; s < q; s++) { ! (k[s] in t) && r.push(k[s]) && (t[k[s]] = "")
            }
            return r.join(",")
        })();
        if (Z.tongji == "on" || D != "" || typeof j != "undefined") {
            if (D != "" && j != "3072") {
                D = D.toString().replace("#", ""); (new Image()).src = Z.bdCommit + "back?url=" + encodeURIComponent(e) + "&sign=" + encodeURIComponent(D) + "&title=" + encodeURIComponent(I.substr(0, 300))
            }
            bdShare.velocity = bdShare.velocity || {};
            bdShare.velocity.mainJsLoaded = +new Date();
            if (typeof j != "undefined") { (new Image()).src = Z.bdCout + "&type=" + j + "&sign=" + D + "&uid=" + Z.bdUid + "&desturl=" + encodeURIComponent(V.referrer) + "&linkid=" + H + "&button=" + W
            } else {
                var p = [0, 0, 0, 0];
                for (var E = 0,
                U = k.length; E < U; E++) {
    ...
    d = {
        create: function() {
            ...
            if (!B) {
                while (D--) {
                    E = J.list(k[D]),
                    T.push('<a href="' + E.url + '" class="' + E.cls + '">' + E.name + "</a>")
                }
                var W = J.html({
                    id: i.bdIds + "_pop",
                    style: "position:" + e,
        ...
        _action: function(W, B, i, U, T) {
            if (B.nodeName == i.toUpperCase()) {
                if (i == "a" || i == "span") {
                    ...
                    E = this;
                    if (D) { (D == "more" && i == "span") ? E._tools() : B.onclick = function(k, e) {
                            return function(p) {
                                var p = p || window.event;
                                var o = p.target || p.srcElement;
                                var n = J.getPosition(o);
        ...
        return {
            init: function() {
                ...
                for (var U = 0; U < W.length; U++) {
                    ...
                    for (var T = 0; T < e.length; T++) { ! (e[T].getAttribute("href")) && e[T].setAttribute("href", "#")
                    }
                }
                ...
                for (var U = 0,
                B = this.t.length; U < B; U++) { (function(i) {
                        var j = i;
                        J.on(E.t[i], "mouseover",
                        function(q) {
            ...
            _share: function() {
                ...
                if (D > 0) {
                    while (D--) {
                        if (i[D].id == "bdshare") {
                            a.bind(i[D], "click", "img",
                            function() {
                                d.create()
                            });
                            a.bind(i[D], "mouseover", "a",
                            function() {
                                d.create()
                            });
